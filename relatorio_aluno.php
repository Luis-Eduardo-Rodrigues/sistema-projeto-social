...
// Buscar escolas e anos
if ($nivelSelecionado == 'fundamental' || $nivelSelecionado == 'todos') {
    // Escolas do Fundamental
    $res_fund = $mysqli->query("SELECT DISTINCT nome_escola AS escola FROM aluno WHERE nome_escola IS NOT NULL AND nome_escola != ''");
    while ($row = $res_fund->fetch_assoc()) {
        $escolas[esc($row['escola'])] = esc($row['escola']);
    }

    // Anos do Fundamental
    $res_anos_fund = $mysqli->query("SELECT DISTINCT ano FROM aluno WHERE ano IS NOT NULL AND ano != '' ORDER BY ano DESC");
...
if ($nivelSelecionado == 'medio' || $nivelSelecionado == 'todos') {
    // Escolas do Médio
    $res_medio = $mysqli->query("SELECT DISTINCT nome_escola_medio AS escola FROM aluno WHERE nome_escola_medio IS NOT NULL AND nome_escola_medio != ''");
    while ($row = $res_medio->fetch_assoc()) {
        $escolas[esc($row['escola'])] = esc($row['escola']);
    }

    // Anos do Médio
    $res_anos_medio = $mysqli->query("SELECT DISTINCT ano_medio FROM aluno WHERE ano_medio IS NOT NULL AND ano_medio != '' ORDER BY ano_medio DESC");
    while ($row = $res_anos_medio->fetch_assoc()) {
        $anos[$row['ano_medio']] = $row['ano_medio'];
    }
}

// Ordena alfabeticamente escolas e anos decrescente
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto px-6 py-10">
        <form action="gerar_pdf_aluno.php" method="POST" class="space-y-8" id="form-relatorio">

            <!-- Campo oculto para o nível -->
            ...
            <div class="grid grid-cols-2 gap-8">
                ...
                <section class="bg-white border border-[#2CA965] rounded-xl p-6 shadow">
                    <h2 class="text-lg font-bold mb-4">Opções de Ensino</h2>
...
<div class="flex justify-between items-center">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Atualiza o valor do campo oculto quando mudar o nível
            ...
            // Lógica do checkbox "Selecionar todas as escolas"
